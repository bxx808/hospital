...
if (isset($_GET['page'])) {
    $page = ($_GET['page'] * 6) - 6;
    $sql = "SELECT * FROM patients WHERE `soft_delete`= 0 LIMIT 6 OFFSET    " . $page;
    $query = $pdo->prepare($sql);
    $query->execute();
} else {
    $sql = "SELECT * FROM patients WHERE `soft_delete`= 0 LIMIT 6 OFFSET 0 ";
    $query = $pdo->prepare($sql);
    $query->execute();
}
...
<div class="row">
    <div class="col-12 d-md-flex">
        ...
        <div class="col-6 d-md-flex justify-content-md-end">
            <form action="recovery_card.php" method="post">
                <button type="submit" class="btn mt-2 btn-outline-primary d-md-flex ">Корзина</button>
            </form>
        </div>
    </div>
</div>
                                    ...
                                    <div class="col-md-8 ">
                                        ...
                                        <div class="function_btn">
                                            <form action="handler/delete_card_handler.php" method="post">
                                                ...
                                                <button class="delet_btn" name="submit"><i
                                                        class="fa-solid fa-trash"></i></button>
                                            </form>
                                            <a class="edit_btn" href="edit_patient.php?id=<?= $patient['id']; ?>"><i
...
<nav aria-label="Page navigation example">
    <ul class="pagination justify-content-center">
        <?php for ($i = 1; $i < $patientsCount + 1; $i++): ?>
